...
//约定常量
var IndicatorValue = [100,101,102,103,104];
var dotSize = 15;
    ...
    function initRiskPoint() {
        ...
        function showResults(results) {
            ...
            dojo.forEach(results, function (result) {
                ...
                var objID = parseInt(attr["OBJECTID"]); //获取要素的objectID
                
                var indexRiskID = keyMapRiskJsonObj[objID]; //由要素ID获取Map的ID
                if (indexRiskID) {
                    var riskResultPointJsonObj = riskResultJsonObj[indexRiskID];    //获取json要素                
                    var riskLevel = riskResultPointJsonObj["DigitResult"];
        ...
        function showResults(results) {
            ...
            for (var i = 0; i < resultCount; i++) {
                ...
                for (var index = 0; index < riskResultJsonObj.length; index++) {
                    var country = riskResultJsonObj[index];
                    if (country["CountryID"] == attr["CountryID"] && country["IndicatorID"] == attr["IndicatorID"]) {
